    ...
    <div className="min-h-screen flex items-center justify-center bg-transparent">
      <div className="w-full max-w-md p-8 space-y-6 bg-gray-800/50 backdrop-blur-sm rounded-2xl shadow-2xl shadow-purple-500/20 border border-purple-500/30">
        <div className="text-center">
          <h1 className="text-4xl font-bold font-orbitron text-purple-400">AETHERIUM GUARD</h1>
          <p className="mt-2 text-gray-300">Login to start your training.</p>
        </div>

        <div className="flex bg-gray-900/50 rounded-lg p-1 border border-gray-700">
            <button onClick={() => setRole('user')} className={`flex-1 capitalize py-2 rounded-md text-sm font-semibold transition-all ${role === 'user' ? 'bg-purple-600' : 'hover:bg-gray-700'}`}>Cadet Login</button>
            <button onClick={() => setRole('admin')} className={`flex-1 capitalize py-2 rounded-md text-sm font-semibold transition-all ${role === 'admin' ? 'bg-purple-600' : 'hover:bg-gray-700'}`}>Admin Login</button>
        </div>
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="relative">
             <input id="password" name="password" type="password" autoComplete="current-password" required value={password} onChange={(e) => setPassword(e.target.value)} className="peer bg-gray-900/50 border border-gray-700 text-white text-sm rounded-lg focus:ring-purple-500 focus:border-purple-500 block w-full p-2.5 placeholder-transparent" placeholder="Password"/>
          ...
          <div>
            <button type="submit" disabled={isLoading || isGoogleLoading} className="group relative w-full flex justify-center py-3 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-purple-600 hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 focus:ring-offset-gray-900 transition-transform duration-200 hover:scale-105 disabled:opacity-50">
              {isLoading ? <span className="w-5 h-5 border-2 border-dashed border-white rounded-full animate-spin"></span> : 'Sign in'}
            </button>
          </div>
        </form>
        ...
        {role === 'user' && (
          <>
            ...
            <button onClick={handleGoogleLogin} disabled={isLoading || isGoogleLoading} className="group relative w-full flex justify-center items-center py-3 px-4 border border-gray-600 text-sm font-medium rounded-md text-white bg-gray-700 hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 focus:ring-offset-gray-900 transition-transform duration-200 hover:scale-105 disabled:opacity-50">
                {isGoogleLoading ? <span className="w-5 h-5 border-2 border-dashed border-white rounded-full animate-spin"></span> : <><GoogleIcon /> Sign in with Google</>}
            </button>
